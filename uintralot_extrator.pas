      ...
      private
            ...
            bParar_Extracao : boolean;
						...
						function gerar_url_por_data_do_jogo(var strUrl_do_Jogo: string;
									strJogo: string; strData_Inicial: string; strData_Final: string
			): boolean;
      function gerar_url_por_numero_do_jogo(var strUrl_do_Jogo: string;
									strJogo: string; iJogo_Inicial: integer; iJogo_Final: integer
			): boolean;
						...
						function Validar_Url_do_Jogo(strUrl: string; link_url: string
									): boolean;
...
function TFrmIntralot_Extrator.gerar_url_por_numero_do_jogo(var strUrl_do_Jogo: string;
...
begin
  ...
  gerar_url_por_data_do_jogo(strUrl_do_jogo, strJogo_Tipo, strData_Inicial, strData_Final);
...
begin
  ...
  if not DirectoryExists(strDiretorio) then begin
...
begin
    ...
    begin
      try
         ...
         lista_de_conteudo_html.Add(objHttp.Get(lista_de_urls[iA], IndyTextEncoding_UTF8));

         objHttp.Disconnect;
      except
            On exc:Exception do
            begin
               if MessageDlg('Erro', 'Erro: ' + exc.Message + #13#10 + 'Continuar.', TMsgDlgType.mtError, [mbYes, mbNO], 0) = mrYes then
                  Dec(iA, 1)
               else
      ...
      if bParar_Extracao then begin
        if MessageDlg('', 'Você deseja realmente, para o download das informações do site da intralot.', tMsgDlgType.mtInformation,
    ...
    //for iA := 0 to lista_de_urls.Count - 1 do begin
      ...
      begin
         ...
         resultado_dos_jogos.SaveToFile('jogo_' + strJogo_Tipo + '_' + strIntervalo +
                                                'pagina_' + Format('%.4d', [iA + 1]) + '.csv');
...
procedure TfrmIntralot_Extrator.FormCloseQuery(Sender: TObject;
...
function TFrmIntralot_Extrator.Paginador(strUrl: string; var lista_de_urls: TStrings): boolean;
var
...
begin
  ...
  // A função retornar o conteúdo html, separado em tokens html.
  ...
  for iA:= 0 to tokens_temp.Count -1 do begin
  ...
  if uIndice > uIndice_Final then begin
    Exit(True);
	end;
  ...
  if not tag_abre_e_fecha(tokens_html, 'div', uIndice, uIndice_Final, uIndice_Tag_Pagination_Final) then
  begin
  ...
  begin
    strErro := 'Tag ''<ul>'' não localizados após o token ' + QuotedStr('"pagination"') + '>';
    Exit(False);
  end;
  ...
  begin
    strErro := 'Tag ''</ul>'' não localizado dentro do tag ''<div class="pagination">'' ... </div>';
    Exit(False);
  end;
  ...
  begin
    ...
    case pagina_estado of
      1:
      begin
        if not((tokens_html[uIndice] = '<li>') and (tokens_html[uIndice + 1] = '1') and (tokens_html[uIndice + 2] = '</li>')) then
        begin
      ...
      3:
        begin
             if not (
                ...
                (tokens_html[uIndice + 12] = '</li>')
                )then
                begin
                  strErro := 'Erro, era esperado os tokens nesta ordem: ' +
                             '''<li style="margin-left:16px;">' + #13#10 +
                             'a href='' + url da próxima página + ''>''' +
                             'número do jogo + ''</a>''';
                Exit(false);
 							 end;
       ...
       4:
         begin
           strErro := 'Era esperado o token ''</div>'' após o token ''</li>'' de da última página';
           Exit(False);
         end;
		end;
...
function TfrmIntralot_Extrator.Validar_Url_do_Jogo(strUrl: string; link_url: string): boolean;
var
			esquerda_url: String;
begin
		  // Vamos retirar as aspas, se não há aspas, indicar erro.
		  if (AnsiLeftStr(link_url, 1) <> '"') or
		  (AnsiRightStr(link_url, 1) <> '"') then begin
      ...
      esquerda_url := LeftStr(link_url, Length(strUrl));
		  ...
		  if esquerda_url <> strUrl then begin
		  strErro := 'Erro: Os primeiros ' + IntToStr(Length(strUrl)) + ' caracteres de ' +
		       link_url + ' devem ser igual a ' + strUrl;
		  Exit(False);
		  end;
...
function TfrmIntralot_Extrator.Tag_Abre_e_Fecha(tokens_html: TStrings; strIdentificador_Tag: string;
